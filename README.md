...
使用 `--export` 指定格式，`--out` 指定输出路径（不指定则默认 `report.md` / `report.json`）。

导出 Markdown 格式：

```bash
st-mem firmware.elf --export md --out report.md
```

导出 JSON 格式：

```bash
st-mem firmware.elf --export json --out data.json
```
...
`--sections` 和 `--export` 可同时使用，终端显示 section 详情，导出写入文件：

```bash
st-mem firmware.elf --sections --export md --out aa.md
```
...
| `--export <fmt>` | 导出报告格式：`json` 或 `md` | - |
| `--out <path>` | 导出文件路径 | `report.json` / `report.md` |
...
# st-mem [--sections] [--width N] [--export FMT --out PATH] runner [--chip X] [--protocol Y]
# ============================================================
# 基础用法（仅分析+烧录，不显示 section 详情）:
runner = "st-mem runner --chip STM32F103C8 --protocol swd"
# 带 section 详情 + 导出 md 到文件:
# runner = "st-mem --sections --export md --out build-report.md runner --chip STM32F103C8 --protocol swd"
# ============================================================
